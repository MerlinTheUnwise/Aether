/**
 * AETHER Server — Execution Tests
 */

import { describe, it, expect, beforeAll, afterAll } from "vitest";
import http from "http";
import { join } from "path";
import { createServer } from "../../src/server/index.js";

const EXAMPLE_PATH = join(process.cwd(), "src/ir/examples/user-registration.json");

function request(server: http.Server, path: string, options: { method?: string; body?: string } = {}): Promise<{ status: number; body: string }> {
  return new Promise((resolve, reject) => {
    const addr = server.address() as { port: number };
    const req = http.request(
      { hostname: "127.0.0.1", port: addr.port, path, method: options.method ?? "GET", headers: options.body ? { "Content-Type": "application/json" } : {} },
      (res) => {
        const chunks: Buffer[] = [];
        res.on("data", (c: Buffer) => chunks.push(c));
        res.on("end", () => resolve({ status: res.statusCode!, body: Buffer.concat(chunks).toString("utf-8") }));
      },
    );
    req.on("error", reject);
    if (options.body) req.write(options.body);
    req.end();
  });
}

describe("Server Execution", () => {
  let server: http.Server;

  beforeAll(async () => {
    server = createServer({ port: 0, graphPath: EXAMPLE_PATH });
    await new Promise<void>((resolve) => server.listen(0, resolve));
  });

  afterAll(async () => {
    await new Promise<void>((resolve) => server.close(() => resolve()));
  });

  it("POST /api/execute returns execution result with wave-by-wave data", async () => {
    const res = await request(server, "/api/execute", { method: "POST" });
    expect(res.status).toBe(200);
    const result = JSON.parse(res.body);
    expect(result).toHaveProperty("outputs");
    expect(result).toHaveProperty("executionLog");
    expect(result).toHaveProperty("waves");
    expect(result).toHaveProperty("nodesExecuted");
    expect(result.waves).toBeGreaterThan(0);
    expect(result.executionLog.length).toBeGreaterThan(0);
  });

  it("POST /api/execute returns contract report", async () => {
    const res = await request(server, "/api/execute", { method: "POST" });
    const result = JSON.parse(res.body);
    expect(result).toHaveProperty("contractReport");
    expect(result.contractReport).toHaveProperty("totalChecked");
    expect(result.contractReport).toHaveProperty("passed");
  });

  it("POST /api/execute-real returns execution result", async () => {
    const res = await request(server, "/api/execute-real", { method: "POST" });
    expect(res.status).toBe(200);
    const result = JSON.parse(res.body);
    expect(result).toHaveProperty("outputs");
    expect(result).toHaveProperty("waves");
  });

  it("execution returns 400 when no graph loaded", async () => {
    const emptyServer = createServer({ port: 0 });
    await new Promise<void>((resolve) => emptyServer.listen(0, resolve));
    const res = await request(emptyServer, "/api/execute", { method: "POST" });
    expect(res.status).toBe(400);
    await new Promise<void>((resolve) => emptyServer.close(() => resolve()));
  });

  it("POST /api/check returns type check result", async () => {
    const res = await request(server, "/api/check", { method: "POST" });
    expect(res.status).toBe(200);
    const result = JSON.parse(res.body);
    expect(result).toBeDefined();
  });

  it("GET /api/dashboard returns full dashboard data", async () => {
    const res = await request(server, "/api/dashboard");
    expect(res.status).toBe(200);
    const data = JSON.parse(res.body);
    expect(data).toHaveProperty("graph");
    expect(data).toHaveProperty("verification");
    expect(data).toHaveProperty("typeSafety");
    expect(data).toHaveProperty("confidence");
    expect(data).toHaveProperty("effects");
    expect(data).toHaveProperty("optimizations");
    expect(data.graph.id).toBe("user_registration");
    expect(data.verification.percentage).toBeGreaterThanOrEqual(0);
  });

  it("POST /api/optimize returns suggestions", async () => {
    const res = await request(server, "/api/optimize", { method: "POST" });
    expect(res.status).toBe(200);
    const suggestions = JSON.parse(res.body);
    expect(Array.isArray(suggestions)).toBe(true);
  });
});
